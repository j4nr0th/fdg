...
gives :math:`u_D`. Because the boundary term is evaluated as the pairing of
the trace of the flux test basis with the mapped values of the datum, no
exactness is expected for arbitrary data at finite order — but since the weak
Dirichlet condition converges to its strong counterpart, the computed
solution approaches the harmonic extension of the boundary data as the order
:math:`p` increases.
...
GEOMETRIES["curved"] = curved_coords
...
def single_mesh() -> Mesh:
    """Build the single-element cubic mesh on the 3x3x3 lattice block."""
    corners = np.array(
        [sum(((d >> k) & 1) * 3**k for k in range(NDIM)) for d in range(8)],
        dtype=np.uint64,
    )
    return Mesh.from_corners(3, corners)


def make_element_map(integ, coords):
...
# face. The load is the metric-free chain integral of the datum against the
# trace of the flux test basis: its reference-domain weights match the
# integration-by-parts structure of the incidence operators exactly, on any
# space map (identity, mirrors, scalings, and curved maps alike).


def solve_laplace(mesh, sm, sq, su, ts, gfun):
    ...
    # saddle matrix.
    nq = int(np.sum(sq.component_dof_counts))
    mq = np.asarray(compute_kform_mass_matrix(sm, sq.order, sq.base_space, sq.base_space))
    mu = np.asarray(compute_kform_mass_matrix(sm, su.order, su.base_space, su.base_space))
    et = np.asarray(incidence_kform_operator(sq, mu, right=True))
    b = np.zeros(nq)
    ...
    # Mirror maps make the mass pairing indefinite (the Jacobian determinant
    # of a mirrored element is negative), so a Cholesky factorization is not
    # an option; use LU factors instead.
    ...
    u_dofs = scipy.linalg.lu_solve(scipy.linalg.lu_factor(schur), et @ rhs_tilde)
    q_dofs = scipy.linalg.lu_solve(factor, b - et.T @ u_dofs)
    return q_dofs, u_dofs
...
def l2_error_u(u_dofs, su, sm_h, ref_fn):
    """Return the L2 difference between the computed solution and ``ref_fn``."""
    nodes = sm_h.integration_space.nodes()
    dof_obj = DegreesOfFreedom(su.get_component_function_space(0), u_dofs)
    ur = reconstruct(dof_obj, *nodes)
    up = transform_kform_to_target(su.order, sm_h, [ur])[0]
...
for label, coords in GEOMETRIES.items():
    ...
    for P in pvals:
        ...
        sm = make_element_map(integ, coords)
        ...
        _, u_dofs = solve_laplace(mesh, sm, sq, su, ts, solution)
        evals.append(l2_error_u(u_dofs, su, sm, solution))
...
def render_curved():
    ...
    sm = make_element_map(integ, GEOMETRIES["curved"])
    bv = FunctionSpace(*(BasisSpecs(BasisType.BERNSTEIN, P) for _ in range(3)))
    su = KFormSpecs(3, bv)
    _, u_dofs = solve_laplace(
        mesh,
        sm,
        KFormSpecs(2, bv),
        su,
    ...
    map_dofs = [
        DegreesOfFreedom(basis_geo, np.asarray(v).ravel())
        for v in GEOMETRIES["curved"](geo_grid)
    ]
    ...
    fine_coords = [np.asarray(reconstruct(dof, *fine_grid)) for dof in map_dofs]
