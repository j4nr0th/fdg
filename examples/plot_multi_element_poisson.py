...
# whose curved edges are rendered exactly, and the error against the
# manufactured solution is sampled on a uniform grid inside the domain.
#
...
# orthogonal slices through the domain, together with the edges of the
# hexahedral elements. The sign of the error is lost; the logarithmic scale
# is needed because the error spans several orders of magnitude. Values below
# :math:`10^{-10}` of the maximum are clamped to the lowest color, so the
# colormap is not dominated by near-zero noise.
#
